...
# ---- openpi_utils launcher config ----
# Embedded submit logic adapted from openpi_utils launcher.
# Vendored helper scripts are expected at:
#   ${SCRIPT_DIR}/scripts/openpi_utils
: "${OPENPI_UTILS_SCRIPT_DIR:=${SCRIPT_DIR}/scripts/openpi_utils}"
: "${OPENPI_SCRIPT_RUN:=${OPENPI_UTILS_SCRIPT_DIR}/script_run.sh}"
: "${OPENPI_DISTRIBUTE_SCRIPT:=${OPENPI_UTILS_SCRIPT_DIR}/script_distribute_slurm_tasks.sh}"
: "${OPENPI_NUM_NODES:=4}"
: "${OPENPI_CPUS_PER_TASK:=224}"
: "${OPENPI_GRES:=gpu:8}"
: "${OPENPI_SBATCH_ARGS:=--partition=2-def --mem=1490000M --time=336:00:00}"
: "${OPENPI_EXTRA_LAUNCH_COMMAND:=pixi run uv run}"
: "${CONFIG_NAME:=}"
: "${OPENPI_CONFIG_YAML:=}"
...
if [ -n "${YAML_GPU_NUM_GPUS}" ]; then
  ...
  if [[ "${OPENPI_NUM_NODES}" =~ ^[0-9]+$ ]] && [[ "${gpus_per_node}" =~ ^[0-9]+$ ]] && [[ "${YAML_GPU_NUM_GPUS}" =~ ^[0-9]+$ ]]; then
    expected_total_gpus=$((OPENPI_NUM_NODES * gpus_per_node))
    if [ "${expected_total_gpus}" -ne "${YAML_GPU_NUM_GPUS}" ]; then
      echo "[WARN]: YAML gpu.num_gpus (${YAML_GPU_NUM_GPUS}) != OPENPI_NUM_NODES*OPENPI_GRES (${OPENPI_NUM_NODES}*${gpus_per_node}=${expected_total_gpus})."
    fi
  fi
fi
...
echo "[INFO]:   .env (launcher/runtime): OPENPI_NUM_NODES=${OPENPI_NUM_NODES}, OPENPI_GRES=${OPENPI_GRES}, OPENPI_RESUME=${OPENPI_RESUME}, OPENPI_OVERWRITE=${OPENPI_OVERWRITE}"
...
if [ ! -f "${OPENPI_DISTRIBUTE_SCRIPT}" ]; then
  echo "[ERROR]: script_distribute_slurm_tasks.sh not found: ${OPENPI_DISTRIBUTE_SCRIPT}"
  exit 1
fi
...
sbatch_args_arr=()
if [ -n "${OPENPI_SBATCH_ARGS:-}" ]; then
  # shellcheck disable=SC2206
  sbatch_args_arr=( ${OPENPI_SBATCH_ARGS} )
fi
...
cmd=(
  ...
  --nodes "${OPENPI_NUM_NODES}"
  --gres "${OPENPI_GRES}"
  --cpus-per-task "${OPENPI_CPUS_PER_TASK}"
  --output "outputs/%x_%j.out"
  --error "outputs/%x_%j.out"
)
if [ "${#sbatch_args_arr[@]}" -gt 0 ]; then
  cmd+=( "${sbatch_args_arr[@]}" )
fi
cmd+=(
  --tasks-per-node 1
  ...
  bash
  -lc
  "${compute_node_launch_cmd}"
)

echo "[INFO]: Running submit command (embedded submit logic):"
printf ' %q' "${cmd[@]}"
echo
"${cmd[@]}"
