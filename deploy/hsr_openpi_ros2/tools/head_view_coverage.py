...
Each object in the library has a distinct flat colour, so counting pixels close
to that colour is enough to say whether it is in frame:
...
def object_colors() -> Dict[str, np.ndarray]:
    """name -> 0..255 RGB, read from the library the spawner uses."""
    ...
    return {o.name: np.array(o.rgba[:3], dtype=float) * 255.0 for o in OBJECT_LIBRARY}
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--tolerance", type=float, default=40.0, help="RGB distance counted as the object")
        ...
        with AnyReader([bag]) as reader:
            ...
            def object_at(stamp: int) -> Optional[str]:
                ...
                for mark_stamp, text in marks:
                    ...
                    if text.startswith("start "):
                        name = next((n for n in colors if n.replace("_", " ") in text), None)
            ...
            for conn, stamp, raw in reader.messages(connections=conns):
                ...
                if image is None:
                    continue
                total += 1
                distance = np.linalg.norm(image.astype(float) - colors[name], axis=2)
                count = int((distance < args.tolerance).sum())
                if count >= args.min_pixels:
                    visible += 1
                shares.append(100.0 * count / image.shape[0] / image.shape[1])

            if not total:
                print(f"{bag.name:<22s} no frames inside an episode")
                continue
            print(
