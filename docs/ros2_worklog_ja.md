...
### 収集の歩留まりがセッション後半で落ちる件（別現象）

上の abort とは**別の話**として、長いチャンクでは後半の成功率が落ちる。
80 本以上回したチャンク 5 本すべてで、最初の 60 数本はほぼ満点、その後崩れる。
始まる位置だけは再現し、最初に失敗したエピソードは 65 / 72 / 74 / 76 / 77
（pick_d_00 / pick_e_00 / pick_d_03 / pick_e_01 / pick_e_02）で、それ以前に
失敗は 1 本も無い。

**abort の前兆ではない。** pick_e_00 は 59.3% まで深く崩れたが 100 本を
完走し、pick_e_01 は 89.3% までしか崩れずに ep 97 で abort した。深く崩れた
方が落ちていない。

機構は分かっていない。潰した仮説:

* マシン負荷 — pick_d_00 / pick_d_03 は他に何も走っていない状態で同じ形に崩れる
* 溶接した物体の蓄積 — 走行中の world のモデルは常に 6 個
* オドメトリのドリフト — 真値との誤差は中央値 0.0000

噛み合わない実測値:

* `cmd_vel` の publish は全区間 6.2 Hz で一定（ループは飢えていない）
* 指令速度は 0.20 → 0.14 m/s に低下（サーボは誤差が *小さい* と見ている）
* オドメトリは正確
* それでも把持時点の `dxy` は 0.14〜0.25 m（許容 0.045）

対処は経験的なもので、チャンクを 50 本に切って崩れ始める前に上がる。歩留まりは
守れるが、崩れ自体も abort も無くならない。
