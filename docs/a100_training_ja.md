...
## 8. 検証 loss は閉ループ性能の判断材料にならない

30,000 step の学習を 10k / 20k / 29,999 の3点で閉ループ評価した結果。
各20エピソード、同じスコアラー、物体と配置は毎回ランダム。

| | 10k | 20k | 29,999 |
| --- | --- | --- | --- |
| 成功率 | **7/20 = 35.0%** | 1/20 = 5.0% | **6/20 = 30.0%** |
| val_loss | 0.0048 | 0.0044 | 0.0115 |
| val_action_l1 | 0.0011 | 0.0009 | **0.0007** |
| gripper never closed | 6 | 6 | **2** |
| 失敗時の物体までの距離・中央値 | 0.093 m | 0.140 m | **0.092 m** |

**開ループ指標は3回とも間違った方向を指した。**

1. 20k: `val_loss` も `action_l1` も 10k より改善しているのに、成功率は 35% → 5%
   に落ちた。事前に決めていた「`val_loss` が 0.011 を超えたら止める」という監視
   条件は発火しなかった。発火しようがない ── 悪化していないので。
2. 29k: `val_loss` が 0.0115 で、初めて 0.011 を超えた。その checkpoint が 30%
   を出した。
3. 同じ 29,999 で `action_l1` は学習中の最良値 0.0007 を記録した。

つまり同一 checkpoint に対して `val_loss` が「過学習」、`action_l1` が「過去最良」
と言い、どちらもタスクの成否とは無関係だった。

理由ははっきりしている。どちらも held-out エピソードの**全フレーム平均**である
のに対し、pick の成否はグリッパが閉じる十数フレームで決まる。平均の中でその区間
は丸め誤差にしかならない。

記録はする。判断には使わない。**checkpoint の良し悪しは閉ループ評価でしか分から
ない**ので、`save_interval` は「評価したい間隔」で決める。

### 20k の谷で打ち切らないこと

20k までしか回していなければ「10k がピーク、以降は劣化の一途」という自信のある
誤った結論になっていた。3点目で谷だと分かった。

説明として使えるのは cosine スケジュールで、learning rate は 10k で 4.0e-5、
20k で 1.7e-5、30k で 5.0e-6。20k は落ち切る途中の測定点にあたる。ただし1 seed
1本の実験なので、スケジュール由来と 20 エピソード評価のばらつき（1本 = 5ポイント）
は分離できていない。**整合する説明であって、示された因果ではない。**

手続き上の教訓のほうが確かで、**学習率が下がり切る前の checkpoint で打ち切って
結論を出さない**、に尽きる。
