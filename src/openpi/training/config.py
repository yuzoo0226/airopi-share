...
class DataConfig:
    ...
    # Use a lighter LeRobot dataset wrapper that skips some heavy checks.
    fast_lerobot: bool = False
    # Select LeRobot data backend (fast loader only).
    lerobot_backend: Literal["parquet", "hf"] = "parquet"
    # Control checks for the fast loader: "all" runs file + timestamp checks,
    # "init_only" runs file checks only, "none" skips checks.
    lerobot_checks: Literal["all", "init_only", "none"] = "none"
    # Optional video backend override (e.g., "pyav").
...
class TaskSamplerConfig:
    # Task sampler mode: "none" uses dataset order/shuffle; "uniform" samples tasks uniformly;
    # "adaptive" prioritizes tasks with higher loss.
    kind: Literal["none", "uniform", "adaptive"] = "none"
    # Multi-process sampling mode:
    # "auto" -> "global_sync" when world_size > 1, otherwise "per_rank".
    # "global_sync" samples global batches then slices per rank.
    # "per_rank" samples independently per rank.
    multi_process_mode: Literal["auto", "global_sync", "per_rank"] = "auto"
    # If true in global_sync mode, avoid duplicated dataset indices across ranks
    # within the same global step when possible.
    disjoint_per_step: bool = True
    # Exponent applied to loss-based weights (adaptive only).
    alpha: float = 1.0
    # EMA decay for per-task loss (adaptive only).
    ema_decay: float = 0.9
    # Minimum probability per task (adaptive only).
    min_prob: float = 0.0
    # Numerical stability epsilon (adaptive only).
    eps: float = 1e-6
    # How to synchronize adaptive task-sampler updates across processes.
    # "global_per_step" keeps the current behavior (per-step global feedback).
    # "local_periodic" updates locally every step and synchronizes sampler state periodically.
    adaptive_sync_mode: Literal["global_per_step", "local_periodic"] = "global_per_step"
    # Synchronization interval (steps) when adaptive_sync_mode="local_periodic".
    adaptive_sync_interval: int = 100
...
class TrainConfig:
    ...
    # Task sampling strategy across multi-task datasets.
    task_sampler: TaskSamplerConfig = dataclasses.field(default_factory=TaskSamplerConfig)
    ...
    num_workers: int = 2
    # Whether to pin host memory in the data loader (useful for faster host-to-device copies).
    pin_memory: bool = False
    # Prefetch factor for the data loader workers (only used when num_workers > 0).
    ...
    # Validation split ratio for episode-based split. If None, no split is applied.
    ...
    # Number of validation batches per evaluation run.
    eval_num_batches: int = 20
    # Number of diffusion steps for action sampling during validation.
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        pytorch_weight_path="/path/to/models/pi0",
    ),
    TrainConfig(
    ...
    TrainConfig(
        ...
        pytorch_weight_path="/path/to/models/pi05",
    ),
    TrainConfig(
        name="pi0_task8",
        model=pi0_config.Pi0Config(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        ...
        #     decay_steps=320_000,  # num_train_stepsに合わせる
    ...
    TrainConfig(
        name="pi05_airoa-hsr-all-v1.0-202504-202512-baseline-28tasks",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=32,  # pi05 is trained with 32-dim actions
            action_horizon=16,
        ...
        data=LeRobotHSRDataConfig(
            repo_id="lerobot_datasets/airoa-hsr-all-v1.0-202504-202512-baseline-28tasks",
            assets=AssetsConfig(
                assets_dir="./assets/pi05_airoa-hsr-all-v1.0-202504-202512-baseline-28tasks",
                asset_id="lerobot_datasets/airoa-hsr-all-v1.0-202504-202512-baseline-28tasks",
            ),
            base_config=DataConfig(
                prompt_from_task=True,
                fast_lerobot=True,
        ...
        lr_schedule=_optimizer.CosineDecaySchedule( # batch 64
            ...
            decay_steps=180_000,  # num_train_stepsに合わせる
